...
│   │       │   └── kustomization.yaml
│   │       └── kustomization.yaml
│   ├── helm/           → Node + monitoring templates
│   │   ├── monitoring/
...
Because Firestore is an external, fully managed database, application data persists independently of Kubernetes pods. When pods are restarted, rescheduled, or scaled, all data remains intact and immediately available to new instances.
...
*** Security
- Kubernetes Service Account (movie-api-sa) is mapped to GCP Service Account
- No service account keys are stored in pods
- Terraform IAM least-privilege model
- Egress is allowed only to TCP port 443 to reach Google APIs / Firestore
- Pods cannot initiate arbitrary outbound connections
- Pods are stateless; data persists across restarts or rescheduling
- GCS bucket stores terraform state outside the cluster
- Firestore creation, IAM, and Workload Identity bindings are managed as code
...
**** Minikube
#+begin_src shell
# Change the service type to ClusterIP (set to LoadBalancer)
sed -i 's/^  type: .*/  type: ClusterIP/' k8s/overlays/base/api-node/manifests/movie-api-service.yaml
...
#+begin_src shell
# Check the IP address using minikube
minikube service list
# Check the IP using kubectl
kubectl get svc

# Check running service from inside of the container
...
# Validate
curl <CLUSTER_IP>:<PORT>/health | jq
...
go-task api-node:deploy-production-cluster

# Access argocd UI via port-forwarding the argocd-server
go-task webui:port-forward-argocd

# The login is 'admin', to retrieve the password then login to the web UI
go-task webui:get-argocd-password

# Delete production cluster
go-task api-node:delete-production-cluster
#+end_src
...
# Install ArgoCD application to monitor prometheus
# if you didn't use tasks that deplyoed ArgoCD with kubectl (like api-node:03-deploy-production-cluster), install argocd with helm
#go-task monitoring:argocd:00-install-argocd-applicationset
go-task monitoring:argocd:01-render-application
go-task monitoring:argocd:02-install-application

# Install prometheus
go-task monitoring:chart:01-render-monitoring
go-task monitoring:chart:02-install-monitoring

# Uninstall ArgoCD and prometheus
go-task monitoring:argocd:03-uninstall-application
go-task monitoring:chart:04-uninstall-monitoring

# Accessing Prometheus and Grafana WebUI
cd cicd/
go-task webui:port-forward-grafana
go-task webui:port-forward-prometheus
#+end_src
...
# Check PVCs for prometheus / grafana / alertmanager
kubectl get pvc -n monitoring

# Check if GCE Ingress controller is enabled - if you don't see output, it means it's disabled
kubectl get pods -n kube-system | grep ingress
# Using gcloud: true - disabled, false - enabled
gcloud container clusters describe <CLUSTER_NAME> \
  --region <REGION> \
  --format="value(addonsConfig.httpLoadBalancing.disabled)"

# Check kubernetes ingress external IP
kubectl get ingress -n <env_namespace>
...
*** Verify GKE cluster and firestore
...
# Add new document to your Node app against Firestore - save the 'id' key from the output
...
- [X] Persistent storage for dashboards
- [ ] Node Exporter
