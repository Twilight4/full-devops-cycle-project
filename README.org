...
Then update backend.tf's =bucket= value to point to the specified bucket name.
...
If you're testing locally in minikube/kind, you need to replace the =values.yaml= with =values-local-testing.yaml= in =k8s/helm/monitoring/kube-prometheus-stack/= because the PVCs in =values.yaml= are bound to GKE-specific volumes.
...
# Install prometheus
go-task monitoring:install-monitoring

# Uninstall prometheus
go-task monitoring:uninstall-monitoring
