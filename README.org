...
# Delete the cluster
go-task minikube:03-delete-cluster
#+end_src
...
*** Test and deploy on GKE using Terraform
**** Testing Terraform infrastructure before deployment
This project includes a test script validate the Terraform configuration by deploying infrastructure to GKE and verifying it behaves as expected.

The CD pipeline also includes testing for production environment if a pull request was issued.
...
1. *Configures Terraform execution*
   - Points to the specified Terraform environment
   - Enables default retry logic to handle transient Terraform or API errors
...
3. *Registers automatic cleanup*
   - Ensures =terraform destroy= is executed at the end of the test
   - Cleanup runs even if the test fails

4. *Fetches Terraform outputs*
   - Reads the =url= output value from Terraform
   - This URL represents the exposed application endpoint

5. *Validates the deployed application*
   - Sends HTTP GET requests to the application endpoint
   - Retries up to 30 times with a 10-second delay between attempts
   - Confirms:
     - HTTP status code is =200=
     - Application responds successfully

6. *Prints response output*
   - Logs the HTTP response body for debugging and visibility

***** Test Success Criteria
The test passes if:
- Terraform applies successfully
- The application endpoint becomes reachable
- The HTTP response returns status =200=

If any of these conditions fail, the test will fail and Terraform resources will still be destroyed.
...
- [ ] Implement network policy for GCP database
- [ ] Implement PersistentVolumeClaims to GCP
