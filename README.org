...
go-task api-node:03-deploy-production-cluster

# Access argocd UI via port-forwarding the argocd-server
go-task webui:01-port-forward-webui

# The login is 'admin', to retrieve the password then login to the web UI
go-task webui:02-get-webui-password

# Delete production cluster
go-task api-node:04-delete-production-cluster
#+end_src
...
cd cicd/
go-task webui:03-port-forward-grafana
go-task webui:04-port-forward-prometheus
#+end_src
