...
# Change directory to terraform staging
cd ../../envs/staging/
...
# Render environment manifests for staging
log_info "Rendering staging manifests..."
if ! go-task argocd:01-render-staging; then
    log_error "Rendering staging manifests failed. Exiting..."
    exit 1
fi
...
# Apply staging env manifest
go-task argocd:02-install-staging
log-info "Deploying staging manifests..."
if ! go-task argocd:02-install-staging; then
    log_error "Deployment of staging manifests failed. Exiting..."
    exit 1
fi
log-info "Staging manifests deployed successfully."
